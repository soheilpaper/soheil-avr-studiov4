...
void N11_SetPixel(unsigned char x, unsigned char y, N11_Pmode mode)
{
		...
		if		 (mode == PIXEL_ON)			SetBit(N11_Cache[x][r], pd);
		else if(mode == PIXEL_OFF)		ClrBit(N11_Cache[x][r], pd);
...
{	
		...
		return(BitIsSet(N11_Cache[x][y /8], (y %8)) ? 0x01 : 0x00);
	  ...
	  height = pgm_read_byte(&bitmap[2]);
		...
		for(xx=0; xx<width; xx++)
		{
			...
			for(yy=0, pc=0; yy<height; yy++,pc++)
			{
				...
				if(mode == PIXEL_XOR)
				{
					if(BitIsSet(Cdata, pc))				N11_Cache[x+xx][rr] &= ~pd;
